...
LOGICAL :: EX,THICKEN_OBSTRUCTIONS,BAD,IDEAL=.FALSE.,SIMPLE_FUEL_DEFINED=.FALSE.,TARGET_PARTICLES_INCLUDED=.FALSE.,&
           SPARK_FOUND=.FALSE.
REAL(EB) :: XB(6),TEXTURE_ORIGIN(3),DT_SPARK=-1._EB,TMP_SPARK=1000._EB
REAL(EB) :: PBX,PBY,PBZ
...
NAMELIST /COMB/ AIT_EXCLUSION_ZONE,AUTO_IGNITION_TEMPERATURE,CHECK_REALIZABILITY,DT_SPARK,EXTINCTION_MODEL,&
                FREE_BURN_TEMPERATURE,FIXED_MIX_TIME,&
                FUEL_C_TO_CO_FRACTION,FUEL_H_TO_H2_FRACTION,HRRPUV_CUT_MAX,HRRPUV_MAX_SMV,HRRPUV_SPARK,&
                ...
                ODE_SOLVER,RADIATIVE_FRACTION,RICHARDSON_ERROR_TOLERANCE,SUPPRESSION,TAU_CHEM,TAU_FLAME,TEMP_MAX_SMV,TMP_SPARK
...
IF (HRRPUV_SPARK<0._EB) HRRPUV_SPARK = (10._EB/CHARACTERISTIC_CELL_SIZE)*1000._EB
IF (DT_SPARK<0._EB) DT_SPARK = 2._EB*CHARACTERISTIC_CELL_SIZE/LAMINAR_FLAME_SPEED
...
CHARACTER(LABEL_LENGTH) :: FUEL,RADCAL_ID='null',SPEC_ID_NU(MAX_SPECIES),SPEC_ID_N_S(MAX_SPECIES),RAMP_CHI_R,FORMULA
...
! Search the input file to find a reserved particle type

CALL SEARCH_INPUT_FILE(LU_INPUT,'FDS SPARK',SPARK_FOUND)
...
! Add particle class for a SPARK igniter

IF (SPARK_FOUND) N_LAGRANGIAN_CLASSES = N_LAGRANGIAN_CLASSES + 1
...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   ELSEIF (SPARK_FOUND) THEN

      CALL SET_PART_DEFAULTS
      WRITE(ID,'(A)') 'FDS SPARK'
      STATIC = .TRUE.
      AGE = 1._EB
      SURF_ID = 'FDS SPARK'
...
SURFACE(SPARK_SURF_INDEX)%ID             = 'FDS SPARK'
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   ! Set properties of reserved SURFs

   IF (N==SPARK_SURF_INDEX) THEN
      ID = 'FDS SPARK'
      IF (SPARK_FOUND) HRRPUA = (HRRPUV_SPARK/1000._EB)*CHARACTERISTIC_CELL_SIZE
      RADIUS = 0.5_EB*CHARACTERISTIC_CELL_SIZE
      GEOMETRY = 'SPHERICAL'
      TMP_FRONT = TMP_SPARK
      TAU_T = 0._EB
      HEAT_TRANSFER_COEFFICIENT = 100._EB
      DO ILPC=1,N_LAGRANGIAN_CLASSES
         IF (LAGRANGIAN_PARTICLE_CLASS(ILPC)%ID=='FDS SPARK') LAGRANGIAN_PARTICLE_CLASS(ILPC)%LIFETIME = DT_SPARK
      ENDDO
   ENDIF
...
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,C_MAX,C_MIN,INITIAL_RADIATION_ITERATIONS,KAPPA0,NMIEANG,NUMBER_RADIATION_ANGLES,&
                ...
                TIME_STEP_INCREMENT,TRI_MODEL,WIDE_BAND_MODEL,MIE_MINIMUM_DIAMETER,MIE_MAXIMUM_DIAMETER,MIE_NDG,&
